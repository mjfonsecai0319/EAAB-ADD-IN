...
internal class DrawPolygonViewModel : BusyViewModelBase
{
    ...
    private readonly GeneratePolygonsUseCase _generatePolygonsUseCase = new();
    ...
    private async Task OnBuildPolygons()
    {
        ...
        try
        {
            var result = await _generatePolygonsUseCase.Invoke(
                workspace: Workspace,
                rootClassPath: FeatureClass!,
                rootClassField: SelectedFeatureClassField!,
                neighborhoodsClassPath: Neighborhood,
                clientsClassPath: ClientsAffected
            );
